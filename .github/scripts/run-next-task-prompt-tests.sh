#!/usr/bin/env bash
# Golden-file tests for build-next-task-agent-prompt.sh
set -euo pipefail
ROOT=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
BUILD="$ROOT/build-next-task-agent-prompt.sh"
TEST_ROOT="$ROOT/test"
FAILED=0
for CASE_DIR in "$TEST_ROOT"/case-*/; do
  [ -d "$CASE_DIR" ] || continue
  ...
  if [ ! -f "$TASK" ] || [ ! -f "$EXPECTED" ] || [ ! -f "$META" ]; then
    echo "SKIP $NAME: missing task.json, expected.txt, or case.json"
    FAILED=1
    continue
  fi
